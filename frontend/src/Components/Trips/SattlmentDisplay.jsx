// src/components/Trips/SettlementsDisplay.jsx
import React, { useEffect, useState } from 'react';
import { useTrip } from '../../Context/TripContext';
import { TrendingUp, TrendingDown, Calculator, RefreshCw } from 'lucide-react';

const SettlementsDisplay = ({ trip }) => {
  const { settlements, fetchSettlements, loading } = useTrip();
  const [balanceSummary, setBalanceSummary] = useState([]);

  const loadSettlements = async () => {
    try {
      const data = await fetchSettlements(trip._id);
        ...
        {balanceSummary.map((balance) => (
          <div key={balance.user.id} className={`p-4 rounded-lg border ${
            ...
            <div className="mt-2 text-xs text-gray-500 space-y-1">
              <p>Paid: {trip.currency} {balance.totalPaid.toFixed(2)}</p>
              <p>Owed: {trip.currency} {balance.totalOwed.toFixed(2)}</p>
            </div>
          </div>
        ))}
      </div>
...
export default SettlementsDisplay;