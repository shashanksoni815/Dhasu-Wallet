// src/components/Groups/GroupMemberManagement.jsx
import React, { useState, useEffect } from 'react';
import { useGroup } from '../../Context/GroupContext';
import { useAuth } from '../../Context/AuthContext';
import { UserPlus, UserMinus, Search, X, Crown, RefreshCw, User, AlertCircle } from 'lucide-react';

const GroupMemberManagement = ({ group, onMembersUpdated }) => {
  const { 
    groupMembers, 
    fetchGroupMembers, 
    searchUsers, 
  ...
  const handleAddMember = async (user) => {
    try {
      setError('');
      await addMemberByEmail(group._id, user.email);
      setSearchEmail('');
      setSearchResults([]);
      setShowSearch(false);
  ...
  // Loading state
  if (membersLoading && !groupMembers.length) {
    return (
  ...
  // Safe access to groupMembers with fallback
              ...
              {searchResults.map(user => (
                <div key={user._id} className="flex items-center justify-between p-3 bg-white border rounded-lg">
                  ...
                  <button
                    ...
                    disabled={membersLoading}
                    className="bg-green-600 text-white px-3 py-1 rounded-lg hover:bg-green-700 text-sm disabled:opacity-50"
                  >
