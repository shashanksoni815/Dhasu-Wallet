// src/components/Trips/EnhancedTripExpenseForm.jsx
import React, { useState, useEffect } from 'react';
import { X, Calendar, Tag, User, Calculator } from 'lucide-react';

const EnhancedTripExpenseForm = ({ trip, expense, onSave, onCancel, isOpen }) => {
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-xl font-semibold text-gray-900">
            {expense ? 'Edit Expense' : 'Add Trip Expense (Auto-Split)'}
          </h2>
          <button
            onClick={onCancel}
          ...
          <div>
            ...
            <div className="mt-1 relative">
              <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                <span className="text-gray-500 sm:text-sm">{trip.currency === 'USD' ? '$' : trip.currency}</span>
              </div>
              <input
                type="number"
                name="amount"
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="text-blue-900 text-lg font-bold">
                    {trip.currency} {sharePerPerson.toFixed(2)}
                  </p>
                </div>
                <div>
...
export default EnhancedTripExpenseForm;